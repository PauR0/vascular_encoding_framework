...
def get_case_directories(cohort_dir, exclude=None, required=None, suffix="", cohort_relative=True):
    ...
    for d in sorted(os.listdir(cohort_dir)):
        ...
        req = (
            "."
            if required is None
            else get_case_convention(required, suffix=suffix, case_dir=full_d)
        )  # '.' Should always exist
...
def load_cohort_object(cohort_dir, which, exclude=None, keys_from_dirs=True, suffix=""):
    ...
    assert which in {"encoding", "centerline"}, error_message(
        f"Can't load object {which} from cohort. Available options are {{'centerline', 'encoding'}}.",
        prnt=False,
    )
    ...
    if which == "encoding":
        ...
        def loader(case_dir, suffix):
            return load_vascular_mesh(path=case_dir, suffix=suffix)

    cases = get_case_directories(
        cohort_dir=cohort_dir, exclude=exclude, required=which, suffix=suffix, cohort_relative=False
    )
    cohort = {}
    for case in cases:
...
def save_cohort_object(cohort_dir, cohort, suffix="", binary=False, overwrite=False):
    ...
    for case, obj in cohort.items():
        ...
        if not isinstance(obj, (vef.VascularAnatomyEncoding, vef.CenterlineTree, vef.VascularMesh)):
            error_message(
        ...
        if isinstance(obj, vef.CenterlineTree):
            save_centerline(
                case_dir=case_dir, cl_tree=obj, suffix=suffix, binary=binary, overwrite=overwrite
            )

        elif isinstance(obj, vef.VascularAnatomyEncoding):
            save_vascular_encoding(
                case_dir=case_dir, vsc_enc=obj, suffix=suffix, binary=binary, overwrite=overwrite
            )

        elif isinstance(obj, vef.VascularMesh):
            save_vascular_mesh(
                vmesh=obj, path=case_dir, suffix=suffix, binary=binary, overwrite=overwrite
            )
...
class _Routinizer:
    ...
    def __init__(self, func, **kwargs) -> None:
        ...
        self.__params = {
            name: deepcopy(kwargs.get(name, param.default))
