...
parser.add_argument("-w", action="store_true", help="""Overwrite existing files.""")
...
if args.mode == "case":
    ...
    if args.encode:
        encode(
            case_dir=case, params=ec_params, binary=args.binary, debug=args.debug, overwrite=args.w
        )
...
if args.mode == "cohort":
    ...
    if args.encode:
        ec_params_fname = (
            args.ec_params
            if args.ec_params is not None
            else os.path.join(cohort_dir, "encoding.json")
        )
        ec_params = read_encoding_config(cohort_dir) if os.path.exists(ec_params_fname) else None
        cohort_run(
            cohort_dir=cohort_dir,
            routine=_Encoder(params=ec_params, binary=args.binary, overwrite=args.w, debug=False),
            exclude=args.exclude,
            n_proc=args.n_proc,
    ...
    if args.align:
        ...
        align_encodings(
            cohort_dir=cohort_dir, params=gpa_params, exclude=args.exclude, overwrite=args.w
        )
