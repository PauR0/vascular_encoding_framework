...
from ..utils._code import Node, Tree, attribute_checker, broadcast_kwargs, check_specific
from ..utils.geometry import polyline_from_points
from ..utils.misc import split_metadata_and_fv
from ..utils.spatial import compute_ref_from_points, get_theta_coord, normalize, radians_to_degrees
...
class ParallelTransport(UniSpline):
    ...
    def compute_parallel_transport_on_centerline(cl, v0) -> ParallelTransport:
        """
        Build the parallel transport of a given vector v0 along a centerline object.

        The parallel transported vector is interpolated
...
class Centerline(UniSpline, Node):
    ...
    def get_tangent(self, t, normalized=True):
        """
        Get the tangent of the centerline at given parameter values.
    ...
    def compute_samples(self, n_samples=None):
        """
        Build the self.centerline_samples attribute.
    ...
    def get_projection_parameter(self, p, method="scalar", full_output=False):
        """
        Compute the value of the parameter for the point in the centerline closest to p.
    ...
    def trim(self, t0_, t1_=None, trim_knots=False, pass_atts=True, n_samps=100) -> Centerline:
        """
        Trim the centerline from t0_ to t1_ and return the new segment as a centerline object.

        If pass_atts is true all the centerline attributes such as the v1, v2 and others are kept.
        The amount of knots for the trimmed centerline will be computed taking into account the
        ...
        def add_to_tree(nid):
            ...
            if parents[nid] != "None":
                ...
                if gr:
                    ...
                    joint = pcl(joint_t)
                    ids = (
                        np.linalg.norm(points - joint, axis=1)
                        > paths[f"path_{nid}"]["radius"][0] * gr
                    )
                    points = np.concatenate(
                        [
                            [joint, pcl((joint_t + pre_joint_t) / 2)],
                            paths[f"path_{nid}"].points[ids],
                        ]
                    )
                else:
                    joint_t = pre_joint_t
